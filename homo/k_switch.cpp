...
void k_switch()
{
  TFHEpp::EvalKey ek1;
  {
    ifstream ifs("../AliceData/ek1.key", ios::binary);
    cereal::PortableBinaryInputArchive ar(ifs);
    ek1.serialize(ar);
    cout << "The Alice key ek1 a été désérialisé." << endl;
  }
  TFHEpp::EvalKey ek2;
  {
    ifstream ifs("../BobData/ek2.key", ios::binary);
    cereal::PortableBinaryInputArchive ar(ifs);
    ek2.serialize(ar);
    cout << "Bob key ek2 a été désérialisé." << endl;
  }
  vector<vector<TLWE_0>> cipher, rka;  
  {
    ifstream ifs{"../AliceData/ciphertext.data", ios::binary};
    cereal::PortableBinaryInputArchive ar(ifs);
    ar(cipher);
  ...
  {
    ifstream ifs{"../AliceData/iSbox.data", ios::binary};
    cereal::PortableBinaryInputArchive ar(ifs);
    ar(iTable);
    cout << "iSbox.data a été désérialisé." << endl;
  }
  {
    ifstream ifs{"../AliceData/mul9.data", ios::binary};
    cereal::PortableBinaryInputArchive ar(ifs);
    ar(Table9);
    cout << "mul9.data a été désérialisé." << endl;
  }
  {
    ifstream ifs{"../AliceData/mul11.data", ios::binary};
    cereal::PortableBinaryInputArchive ar(ifs);
    ar(Table11);
    cout << "mul11.data a été désérialisé." << endl;
  }
  {
    ifstream ifs{"../AliceData/mul13.data", ios::binary};
    cereal::PortableBinaryInputArchive ar(ifs);
    ar(Table13);
    cout << "mul13.data a été désérialisé." << endl;
  }
  {
    ifstream ifs{"../AliceData/mul14.data", ios::binary};
    cereal::PortableBinaryInputArchive ar(ifs);
    ar(Table14);
    cout << "mul14.data a été désérialisé." << endl;
  }
  {
    ifstream ifs{"../AliceData/ka.key", ios::binary};
    cereal::PortableBinaryInputArchive ar(ifs);
    ar(rka);
...
/////////////////////////////// CipheriShiftRows
  ...
  for (int i = 0; i < 16; i++)
  {
    for (int j = 0; j < 8; j++)
    {
      TFHEpp::SM4_CircuitBootstrappingFFT<iksP, bkP, privksP>(bootedTGSW[i][j], cipher[i][j], ek1);
    }
  }
...
/////////////////////////////// CipheriSubBytes
  ...
  for (int i = 0; i < 16; i++)
  {
    ...
    {
    // level 1 -> level 0
    TFHEpp::IdentityKeySwitch<iksP>(B[i][j], iSbox_value[i][j], ek1.getiksk<iksP>());
    }
  }
...
/////////////////////////////// begin CipheriMixColumns
  for (int i = 0; i < 16; i++)
  {
    for (int j = 0; j < 8; j++)  {
      TFHEpp::SM4_CircuitBootstrappingFFT<iksP, bkP, privksP>(bootedTGSW[i][j], B[i][j], ek1);
    }
  }
  ...
  for (int i = 0; i < 16; i++) {          
    for (int j = 0; j < 8; j++) {      
      TFHEpp::IdentityKeySwitch<iksP>(mul9[i][j], mul9_value[i][j], ek1.getiksk<iksP>());           
      TFHEpp::IdentityKeySwitch<iksP>(mul11[i][j], mul11_value[i][j], ek1.getiksk<iksP>());
      TFHEpp::IdentityKeySwitch<iksP>(mul13[i][j], mul13_value[i][j], ek1.getiksk<iksP>());
      TFHEpp::IdentityKeySwitch<iksP>(mul14[i][j], mul14_value[i][j], ek1.getiksk<iksP>());         
    }          
  }
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  {
    ifstream ifs{"../BobData/kb.key", std::ios::binary};
    cereal::PortableBinaryInputArchive ar(ifs);
    ar(rkb);
  ...
  {
    ifstream ifs{"../BobData/Sbox.data", ios::binary};
    cereal::PortableBinaryInputArchive ar(ifs);
    ar(Table);
  ...
  {
    ifstream ifs{"../BobData/consByte.data", ios::binary};
    cereal::PortableBinaryInputArchive ar(ifs);
    ar(consByte);
...
/////////////////////////////////////
  ...
  for (int i = 1; i < 10; i++)
  {
        ...
        for (int i = 0; i < 16; i++)
        {
            ...
            {
                // level 1 -> level 0
                TFHEpp::IdentityKeySwitch<iksP>(cipher[i][j], Sbox_value[i][j], ek2.getiksk<iksP>());
            }
        }
