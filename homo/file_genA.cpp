...
void file_genA()
{
  ...
  TFHEpp::EvalKey ek1;
  ek1.emplaceiksk<iksP>(*sk1);
  ek1.emplacebkfft<bkP>(*sk1);
  ek1.emplaceprivksk4cb<privksP>(*sk1);
  ek1.emplacebkfft<TFHEpp::lvl01param>(*sk1); // used for identitybootstrapping


  std::filesystem::create_directories("../AliceData");
  // export the Alice key to file for later use
  {
    std::ofstream ofs{"../AliceData/sk1.key", std::ios::binary};
    cereal::PortableBinaryOutputArchive ar(ofs);
    sk1->serialize(ar);
    std::cout << "The Alice SecretKey has been serialized." << std::endl;
  };

  // export the Alice key to a file (for the cloud)
  {
    std::ofstream ofs{"../AliceData/ek1.key", std::ios::binary};
    cereal::PortableBinaryOutputArchive ar(ofs);
    ek1.serialize(ar);
    std::cout << "The Alice key ek1 has been serialized." << std::endl;
  };


//=========================================================================================
  int i, Nr=0, Nk=0, NN=128;
  unsigned char ka[16], plain[16], RoundKeya[240];
  ...
  std::vector<std::vector<TLWE_0>> rka;
  rka.resize(240);
  for (int i = 0; i < 240; i++)
  {
    int bin_ka[8];
    ...
    for (int j = 0; j < 8; j++)
    {
      rka[i][j] = TFHEpp::tlweSymIntEncrypt<typename bkP::domainP>((typename bkP::domainP::T)bin_ka[j], bkP::domainP::alpha,
                                                                    sk1->key.get<typename bkP::domainP>());
    }
  }
  ...
  {
    std::ofstream ofs{"../AliceData/ka.key", std::ios::binary};
    cereal::PortableBinaryOutputArchive ar(ofs);
    ar(rka);
    std::cout << "The ka key has been serialized." << std::endl;
  };
  {
    std::ofstream ofs{"../AliceData/ciphertext.data", std::ios::binary};
    cereal::PortableBinaryOutputArchive ar(ofs);
    ar(cipher);
  ...
  std::vector<TRLWE_1> iTable(2), Table9(2), Table11(2), Table13(2), Table14(2) ;
  MakeTable(iTable, sk1->key.get<privksP::targetP>(), iSbox); //Utiliser la clé TRLWE de niveau 1
  MakeTable(Table9, sk1->key.get<privksP::targetP>(), mul9);
  MakeTable(Table11, sk1->key.get<privksP::targetP>(), mul11);
  MakeTable(Table13, sk1->key.get<privksP::targetP>(), mul13);
  MakeTable(Table14, sk1->key.get<privksP::targetP>(), mul14);
  {
    std::ofstream ofs{"../AliceData/iSbox.data", std::ios::binary};
    cereal::PortableBinaryOutputArchive ar(ofs);
    ar(iTable);
    std::cout << "The iSbox  has been serialized." << std::endl;
  };
  {
    std::ofstream ofs{"../AliceData/mul9.data", std::ios::binary};
    cereal::PortableBinaryOutputArchive ar(ofs);
    ar(Table9);
    std::cout << "The mul9  has been serialized." << std::endl;
  };
  {
    std::ofstream ofs{"../AliceData/mul11.data", std::ios::binary};
    cereal::PortableBinaryOutputArchive ar(ofs);
    ar(Table11);
    std::cout << "The mul11  has been serialized." << std::endl;
  };
  {
    std::ofstream ofs{"../AliceData/mul13.data", std::ios::binary};
    cereal::PortableBinaryOutputArchive ar(ofs);
    ar(Table13);
    std::cout << "The mul13  has been serialized." << std::endl;
  };
  {
    std::ofstream ofs{"../AliceData/mul14.data", std::ios::binary};
    cereal::PortableBinaryOutputArchive ar(ofs);
    ar(Table14);
