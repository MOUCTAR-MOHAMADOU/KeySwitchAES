...
void file_genB()
{
  ...
  TFHEpp::EvalKey ek2;
  ek2.emplaceiksk<iksP>(*sk2);
  ek2.emplacebkfft<bkP>(*sk2);
  ek2.emplaceprivksk4cb<privksP>(*sk2);
  ek2.emplacebkfft<TFHEpp::lvl01param>(*sk2); // used for identitybootstrapping

  std::filesystem::create_directories("../BobData");
  // export the  Bob secret key to file for later use
  {
    std::ofstream ofs{"../BobData/sk2.key", std::ios::binary};
    cereal::PortableBinaryOutputArchive ar(ofs);
    sk2->serialize(ar);
    std::cout << "The Bob SecretKey has been serialized." << std::endl;
  };

  // export the Bob key to a file (for the cloud)
  {
    std::ofstream ofs{"../BobData/ek2.key", std::ios::binary};
    cereal::PortableBinaryOutputArchive ar(ofs);
    ek2.serialize(ar);
    std::cout << "The Bob key ek2 has been serialized." << std::endl;
  };


//=========================================================================================
  int i, Nr=0, Nk=0, NN=128;
  unsigned char kb[16], RoundKeyb[240];

  // Calculate Nk and Nr from the received value.
  Nk = NN / 32;  //4
  Nr = Nk + 6;   //10

    printf("Entrez la clé Bob ka en hexadécimal : ");
  for(i=0; i<16; i++)
  {
  scanf("%0hhx",&kb[i]);
  }
  ...
  std::vector<std::vector<TLWE_0>> rkb;
  rkb.resize(240);
  for (int i = 0; i < 240; i++)
  {
    int bin_kb[8];
    ...
    for (int j = 0; j < 8; j++)
    {
      rkb[i][j] = TFHEpp::tlweSymIntEncrypt<typename bkP::domainP>((typename bkP::domainP::T)bin_kb[j], bkP::domainP::alpha,
                                                                    sk2->key.get<typename bkP::domainP>());
    }
  }
  ...
  {
    std::ofstream ofs{"../BobData/kb.key", std::ios::binary};
    cereal::PortableBinaryOutputArchive ar(ofs);
    ar(rkb);
    std::cout << "The kb key has been serialized." << std::endl;
  };
  {
    std::ofstream ofs{"../BobData/consByte.data", std::ios::binary};
    cereal::PortableBinaryOutputArchive ar(ofs);
    ar(consByte);
  ...
  MakeTable(Table, sk2->key.get<privksP::targetP>(), Sbox); // Utiliser la clé TRLWE de niveau 1
  {
    std::ofstream ofs{"../BobData/Sbox.data", std::ios::binary};
    cereal::PortableBinaryOutputArchive ar(ofs);
    ar(Table);
